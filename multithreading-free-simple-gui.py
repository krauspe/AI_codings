...
def execute_command(command, window):
    """Execute a PowerShell command and display the output."""
    try:
        result = subprocess.run(["powershell", "-Command", command], capture_output=True, text=True)
        output = result.stdout if result.returncode == 0 else result.stderr
        window.write_event_value('-COMMAND_OUTPUT-', output)
...
layout = [
    ...
    [sg.Multiline('', size=(80, 20), font=("Courier", 12), key='-OUTPUT-', disabled=True, autoscroll=True)],
    [sg.Button('Command', font=("Helvetica", 16)), sg.Button('Exit', font=("Helvetica", 16))]
]
...
while True:
    ...
    if event == sg.WINDOW_CLOSED or event == 'Exit':
        ...
        command = sg.popup_get_text("Enter a PowerShell command:", "Command Input")
        if command:
